...
- Aggregate domain vs per-node structural metrics: the byte engine is **one** cluster-wide compaction domain (RF folded in), so the byte *totals* — disk, live, expired, tombstones — are additive across nodes and come straight from it. But three per-tick figures describe the *shape* of a store, not a byte total, and a real cluster has one store **per node**: `sstableCount`, `readSstables` (read amp) and `compactionBytes` (write amp → the saturation backlog). All three are reconstructed by re-running one average node's `1/nodes` share (`skew: undefined`, so no recursion) through the same engine and combining — `sstableCount` and `compactionBytes` as fleet totals (per-node × nodes), `readSstables` as the per-node figure itself (a read hits one replica's store). This is **not** a flat × nodes: without compaction the structure is volume-driven (fleet count grows with the ring, read amp *falls*, and it collapses back to the aggregate at nodes = 1); under TWCS it is time-structural and exact; under STCS a node with `1/nodes` the data carries slightly fewer tiers — which is exactly why the compaction bill must be measured per node, not on the aggregate stream. Reading write-amp off the aggregate (a store `nodes`× larger, an extra tier) is what had inflated the "Compaction throttled" saturation verdict from a true ρ ≈ 0.99 to a false ρ ≈ 1.15 (fixed; preset re-tuned to a 7 MiB/s cap to stay a genuine fatal). Byte totals and the skew/disk figures are untouched by this — goldens moved only in `readSstables`/`compactionBytes`/`compactionBacklogBytes`.
- Schema shape: columns are **one unified list**; a column with `key: true` is a clustering-key component (value bytes only, no per-cell overhead, and the only field a row tombstone carries), everything else is a regular cell (`value + cellOverhead`). There is no separate `clusteringKeyBytes` scalar — legacy `#c=` links that still carry one are migrated into a key column by `clampScenario`. The distinction is real Cassandra storage (clustering prefix vs. timestamped cell), so don't collapse it into one number.
- Partition key vs clustering key: the **partition** key is a separate scalar `schema.partitionKeyBytes` (default off), *not* a column role, because it is stored **once per partition**, not per row. `buildSizeModel` returns `partitionKeyOnDiskBytes` (per-partition, compressed × RF); `toSimConfig` multiplies by `partitionCount` into `SimConfig.partitionOverheadBytes`, a **flat constant** the engine adds to the emitted `liveBytes`/`diskBytes` (internal totals stay pure, so aging/compaction/skew see only real row bytes) whenever `diskBytes > 0`. It never enters `onDiskRowBytes` — folding a per-partition cost into per-row size over-counts by the rows-per-partition factor (millions on a coarse key). Because `partitionCount` is static, the term does not grow with ingestion; it is below the noise floor in every preset (500 keys × 16 B ≈ 12 KB), so goldens/presets are unchanged with it off. The clustering key stays per-row (`key: true`).
